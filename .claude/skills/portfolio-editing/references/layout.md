...
- **Homepage projects:** the **BUILDING PRODUCTS USED BY MILLIONS** label is hidden. Projects are a single source-ordered column (not the even/odd two-column masonry), with **descriptions hidden**. The list starts `24px` below the nav. The title/tag block is **stacked**: the title sits on its own line, and the tag pills sit on a left-aligned wrapping row `12px` below it — one consistent pattern for every card, whether its tag string is short or long (desktop keeps the single title-left / tags-right row).
...
| Title ↔ tag | space-between (auto), min 16px | stacked — tags on their own row, 12px below the title, left-aligned |
